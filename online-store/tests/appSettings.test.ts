...
describe('AppSettings\'s "saveSettings" method', (): void => {
  ...
  const isObjectPresent = Object.keys(localStorage).some(item => item === settings['localStorageName']);
  const settingsFromStorage = JSON.parse(localStorage.getItem(settings['localStorageName']) as string);
